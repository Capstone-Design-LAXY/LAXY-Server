...
import com.group.laxyapp.dto.user.UserCntDto;
...
public class UserService {
    ...
    public BindingResult joinValid(UserRegistRequest req, BindingResult bindingResult)
    {
    ...
    public void join(UserRegistRequest req) {
    ...
    @Transactional
    public void changeRole(Long userId, Authentication auth) {
        User user = userRepository.findById(userId).orElseThrow(() -> new IllegalArgumentException("User not found"));
        user.rankUp(user.getUserRole(), auth); // 역할을 실제로 업데이트 할지 확인
    }
    ...
    public UserCntDto getUserCnt() {
        return UserCntDto.builder()
                ...
                .totalL5Cnt(userRepository.countAllByUserRole(UserRole.L5))
                .totalL4Cnt(userRepository.countAllByUserRole(UserRole.L4))
                .totalL3Cnt(userRepository.countAllByUserRole(UserRole.L3))
                .totalL2Cnt(userRepository.countAllByUserRole(UserRole.L2))
                .totalL1Cnt(userRepository.countAllByUserRole(UserRole.L1))
                .build();
    }
}
